...
engine_mac: src/*.cpp
	$(CXX) -std=c++11 -Iheader/ext/freetype2 src/*.cpp lib/*.a -framework OpenGL -framework GLUT -L/usr/local/Cellar/freetype/2.6_1/lib -lfreetype -framework OpenAL -o ./build/my-engine

editor_mac: src/*.cpp
	$(CXX) -std=c++11 -DEDITOR -Iheader/ext/freetype2 src/*.cpp lib/*.a -framework OpenGL -framework GLUT -L/usr/local/Cellar/freetype/2.6_1/lib -lfreetype -framework OpenAL -o ./build/my-engine-editor
	
testing_mac: src/*.cpp test/testing.cpp
	$(CXX) -std=c++11 -DTESTING -g -fprofile-arcs -ftest-coverage -Iheader/ext/freetype2 src/*.cpp test/testing.cpp lib/*.a -framework OpenGL -framework GLUT -L/usr/local/Cellar/freetype/2.6_1/lib -lfreetype -framework OpenAL -o ./build/my-engine-test
	./build/my-engine-test
